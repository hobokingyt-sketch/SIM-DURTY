...
func configure_work(definition: SkeletonWorkDefinition) -> void:
	...
	%WorkTerms.text = "+%s  /  %d minutes" % [
		money_text(definition.payout_cents), definition.duration_minutes,
	]
...
func show_state(state: Dictionary, work_available: bool, dirty: bool) -> void:
	...
	time_label.text = "Day %d · %02d:%02d" % [
		1 + floori(clock_minutes / 1440.0),
		floori((clock_minutes % 1440) / 60.0), clock_minutes % 60,
	]
