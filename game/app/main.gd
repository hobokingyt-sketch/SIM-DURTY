...
func _ready() -> void:
	...
	print("[SIM-DURTY] Visual R3 control craft boot OK | build=%s" % BuildInfo.build_id())
	print(debug_report())
	if not app_mode.is_empty():
...
func debug_report() -> String:
	...
	return DebugReport.compose({
		"milestone": "6D-R3 Button and control craft", "save_schema": SkeletonSave.SCHEMA_VERSION,
