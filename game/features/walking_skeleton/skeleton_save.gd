...
func write_state(state: Dictionary) -> Error:
	if not _valid_path() or not SkeletonSession.is_valid_snapshot(state):
		return ERR_INVALID_DATA
	# Do not silently replace an unreadable, corrupt, or newer-version save.
	...
	file.store_string(JSON.stringify(envelope(state), "\t", true))
	file.flush()
	...
	if FileAccess.file_exists(_path):
		var backup_error: Error = DirAccess.copy_absolute(
			...
			ProjectSettings.globalize_path(_path + ".bak")
		)
		if backup_error != OK:
			return backup_error
	# Same-directory replacement: primary is never truncated before staged validation.
...
func _valid_path() -> bool:
	return _path.begins_with("user://") and not _path.contains("..") \
		and _path.ends_with(".json")
...
static func decode(text: String) -> Dictionary:
	...
	if data.get("activity_id") != ACTIVITY_ID \
			or not SkeletonSession.is_valid_snapshot(data.get("state")):
		return {"error": ERR_FILE_CORRUPT, "state": {}}
