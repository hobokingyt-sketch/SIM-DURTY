...
func _run() -> void:
	print("[tests] SIM-DURTY Controls 6D.3")
	...
	var frame_suite: RefCounted = FrameTests.new()
	failures += await frame_suite.run(self)
	var control_suite: RefCounted = ControlTests.new()
